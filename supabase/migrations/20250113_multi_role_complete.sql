...
BEGIN 
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'users_approval_status_check'
  ) THEN
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ Users table altered successfully';
...
CREATE TABLE IF NOT EXISTS role_requests (
  ...
  -- Foreign keys
  CONSTRAINT role_requests_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  CONSTRAINT role_requests_reviewed_by_fkey 
    FOREIGN KEY (reviewed_by) REFERENCES users(id),
  -- Check constraints
  CONSTRAINT role_requests_requested_role_check 
    CHECK (requested_role IN ('panitia', 'ustadz', 'admin')),
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ role_requests table created';
END $migration$;
...
CREATE TABLE IF NOT EXISTS admin_invites (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  -- Foreign key
  CONSTRAINT admin_invites_invited_by_fkey 
    FOREIGN KEY (invited_by) REFERENCES users(id),
  -- Check constraints
  CONSTRAINT admin_invites_role_check 
    CHECK (role IN ('panitia', 'ustadz', 'admin')),
  CONSTRAINT admin_invites_status_check 
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ admin_invites table created';
END $migration$;
...
CREATE TABLE IF NOT EXISTS note_shares (
  ...
  CONSTRAINT note_shares_shared_by_fkey 
    FOREIGN KEY (shared_by) REFERENCES users(id),
  CONSTRAINT note_shares_shared_to_fkey 
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ note_shares table created';
END $migration$;
...
CREATE TABLE IF NOT EXISTS note_share_batches (
  ...
  -- Foreign keys
  CONSTRAINT note_share_batches_note_id_fkey 
    FOREIGN KEY (note_id) REFERENCES notes(id) ON DELETE CASCADE,
  CONSTRAINT note_share_batches_shared_by_fkey 
    FOREIGN KEY (shared_by) REFERENCES users(id),
  -- Check constraint
  CONSTRAINT note_share_batches_target_type_check 
    CHECK (target_type IN ('specific_users', 'all_members', 'by_role'))
);
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ note_share_batches table created';
END $migration$;
...
CREATE TABLE IF NOT EXISTS role_features (
  ...
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  -- Unique combination
  CONSTRAINT role_features_unique_config 
    UNIQUE (role, subscription_tier, feature_key)
);
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ role_features table created';
END $migration$;
...
ALTER TABLE imagekit_temp_uploads 
ADD COLUMN IF NOT EXISTS uploader_role TEXT DEFAULT 'member';
...
BEGIN 
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'imagekit_temp_uploads_uploader_role_check'
  ) THEN
    ALTER TABLE imagekit_temp_uploads 
    ADD CONSTRAINT imagekit_temp_uploads_uploader_role_check 
    CHECK (uploader_role IN ('admin', 'panitia', 'ustadz', 'member'));
  END IF;
END $migration$;
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ imagekit_temp_uploads table altered';
END $migration$;
...
CREATE INDEX IF NOT EXISTS idx_admin_invites_status_expires ON admin_invites (status, expires_at) WHERE status = 'pending';
...
CREATE INDEX IF NOT EXISTS idx_note_shares_read_at ON note_shares (read_at) WHERE read_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_note_share_batches_completed_at ON note_share_batches (completed_at) WHERE completed_at IS NULL;
...
DO $migration$ 
BEGIN 
  RAISE NOTICE '✅ Created 23 performance indexes';
END $migration$;
...
CREATE TRIGGER trigger_update_role_requests_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION update_role_requests_updated_at();
...
CREATE TRIGGER trigger_update_role_features_updated_at
...
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('member', 'free', 'max_notes', '{"limit": 10}', 'Maximum notes for free members'),
  ('member', 'free', 'imagekit_retention_hours', '{"hours": 1}', 'PDF retention time (1 hour)'),
  ('member', 'free', 'can_share_notes', '{"enabled": false}', 'Cannot share notes'),
  ('member', 'free', 'ai_summary', '{"enabled": false}', 'No AI summary access')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Member features (premium tier)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('member', 'premium', 'max_notes', '{"limit": 100}', 'Maximum notes for premium members'),
  ('member', 'premium', 'imagekit_retention_hours', '{"hours": 1}', 'PDF retention time (1 hour)'),
  ('member', 'premium', 'can_share_notes', '{"enabled": false}', 'Cannot share notes'),
  ('member', 'premium', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Member features (advance tier)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('member', 'advance', 'max_notes', '{"limit": -1}', 'Unlimited notes'),
  ('member', 'advance', 'imagekit_retention_hours', '{"hours": 1}', 'PDF retention time (1 hour)'),
  ('member', 'advance', 'can_share_notes', '{"enabled": false}', 'Cannot share notes'),
  ('member', 'advance', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Panitia features (free tier)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('panitia', 'free', 'max_notes', '{"limit": 500}', 'Maximum notes for free panitia'),
  ('panitia', 'free', 'imagekit_retention_hours', '{"hours": 72}', 'PDF retention time (3 days)'),
  ('panitia', 'free', 'can_share_notes', '{"enabled": true}', 'Can share notes'),
  ('panitia', 'free', 'max_share_recipients', '{"limit": 100}', 'Max 100 recipients per share'),
  ('panitia', 'free', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Panitia features (panitia_basic tier)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('panitia', 'panitia_basic', 'max_notes', '{"limit": 2000}', 'Maximum notes for basic panitia'),
  ('panitia', 'panitia_basic', 'imagekit_retention_hours', '{"hours": 168}', 'PDF retention time (7 days)'),
  ('panitia', 'panitia_basic', 'can_share_notes', '{"enabled": true}', 'Can share notes'),
  ('panitia', 'panitia_basic', 'max_share_recipients', '{"limit": 500}', 'Max 500 recipients per share'),
  ('panitia', 'panitia_basic', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Panitia features (panitia_pro tier)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('panitia', 'panitia_pro', 'max_notes', '{"limit": -1}', 'Unlimited notes'),
  ('panitia', 'panitia_pro', 'imagekit_retention_hours', '{"hours": 720}', 'PDF retention time (30 days)'),
  ('panitia', 'panitia_pro', 'can_share_notes', '{"enabled": true}', 'Can share notes'),
  ('panitia', 'panitia_pro', 'max_share_recipients', '{"limit": 2000}', 'Max 2000 recipients per share'),
  ('panitia', 'panitia_pro', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Admin features (unlimited everything)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('admin', 'free', 'max_notes', '{"limit": -1}', 'Unlimited notes'),
  ('admin', 'free', 'imagekit_retention_hours', '{"hours": 720}', 'PDF retention time (30 days)'),
  ('admin', 'free', 'can_share_notes', '{"enabled": true}', 'Can share notes'),
  ('admin', 'free', 'max_share_recipients', '{"limit": -1}', 'Unlimited recipients'),
  ('admin', 'free', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;

-- Ustadz features (similar to panitia for now)
INSERT INTO role_features (role, subscription_tier, feature_key, feature_value, description)
VALUES
  ('ustadz', 'free', 'max_notes', '{"limit": 1000}', 'Maximum notes for ustadz'),
  ('ustadz', 'free', 'imagekit_retention_hours', '{"hours": 168}', 'PDF retention time (7 days)'),
  ('ustadz', 'free', 'can_share_notes', '{"enabled": true}', 'Can share notes'),
  ('ustadz', 'free', 'max_share_recipients', '{"limit": 500}', 'Max 500 recipients per share'),
  ('ustadz', 'free', 'ai_summary', '{"enabled": true}', 'AI summary enabled')
ON CONFLICT (role, subscription_tier, feature_key) DO NOTHING;
...
BEGIN
  SELECT COUNT(*) INTO feature_count FROM role_features;
  RAISE NOTICE '✅ Inserted role_features (total: %)', feature_count;
END $migration$;
...
UPDATE imagekit_temp_uploads itu
SET uploader_role = u.role
FROM users u
WHERE itu.user_id = u.auth_user_id
...
BEGIN
  ...
  FROM users 
  WHERE approval_status = 'approved' AND role = 'member';
  
  SELECT COUNT(*) INTO updated_uploads 
  FROM imagekit_temp_uploads 
  WHERE uploader_role != 'member';
  
  RAISE NOTICE '✅ Updated % existing users to approved status', updated_users;
  RAISE NOTICE '✅ Updated % imagekit uploads with uploader role', updated_uploads;
END $migration$;
...
ALTER TABLE note_share_batches ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY admin_invites_admin_full ON admin_invites
  FOR ALL USING (
    EXISTS (SELECT 1 FROM users WHERE auth_user_id = auth.uid() AND role = 'admin')
  );

-- Anyone can verify invite code (for registration)
CREATE POLICY admin_invites_verify_public ON admin_invites
  FOR SELECT USING (
...
CREATE POLICY note_shares_select_received ON note_shares
  FOR SELECT USING (
    shared_to IN (SELECT id FROM users WHERE auth_user_id = auth.uid())
  );

-- Users can view shares they created
CREATE POLICY note_shares_select_sent ON note_shares
  FOR SELECT USING (
    shared_by IN (SELECT id FROM users WHERE auth_user_id = auth.uid())
  );

-- Panitia/Admin can create shares
CREATE POLICY note_shares_insert_staff ON note_shares
  FOR INSERT WITH CHECK (
    shared_by IN (
...
CREATE POLICY note_shares_update_received ON note_shares
  FOR UPDATE USING (
    shared_to IN (SELECT id FROM users WHERE auth_user_id = auth.uid())
  );

-- Users can delete shares they created
CREATE POLICY note_shares_delete_own ON note_shares
  FOR DELETE USING (
    shared_by IN (SELECT id FROM users WHERE auth_user_id = auth.uid())
  );
...
CREATE POLICY note_share_batches_select_own ON note_share_batches
  FOR SELECT USING (
    shared_by IN (SELECT id FROM users WHERE auth_user_id = auth.uid())
  );

-- Panitia/Admin can create batches
CREATE POLICY note_share_batches_insert_staff ON note_share_batches
  FOR INSERT WITH CHECK (
    shared_by IN (
      SELECT id FROM users 
      WHERE auth_user_id = auth.uid() 
...
CREATE POLICY note_share_batches_update_system ON note_share_batches
...
CREATE POLICY role_features_select_all ON role_features
  FOR SELECT USING (true);

-- Only admins can modify features
CREATE POLICY role_features_modify_admin ON role_features
  FOR ALL USING (
    EXISTS (SELECT 1 FROM users WHERE auth_user_id = auth.uid() AND role = 'admin')
  );
...
GRANT SELECT ON TABLE admin_invites TO anon;
GRANT SELECT ON TABLE role_features TO anon;
