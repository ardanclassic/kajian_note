...
serve(async (req)=>{
  ...
  try {
    ...
    if (text.startsWith("/verify ")) {
      ...
      const { data: user, error } = await supabase.from("users").select("id, username, full_name").eq("username", kajianUsername).single();
      if (error || !user) {
        await sendTelegramMessage(chatId, `❌ Username "${kajianUsername}" tidak ditemukan.\n\nPastikan username Anda benar.`);
        return new Response("OK", {
          status: 200,
          headers: corsHeaders
        });
      ...
      // Success!
      await sendTelegramMessage(chatId, `✅ Verifikasi Berhasil!\n\n` + `Username: ${user.username}\n` + `Nama: ${user.full_name}\n` + `Chat ID: ${chatId}\n\n` + `Anda sekarang dapat menerima PDF catatan dari Kajian Note!`);
      return new Response("OK", {
        status: 200,
        headers: corsHeaders
      });
    ...
    if (text === "/status") {
      ...
      const supabase = createClient(Deno.env.get("SUPABASE_URL"), Deno.env.get("SUPABASE_SERVICE_ROLE_KEY"));
      const { data: user } = await supabase.from("users").select("username, full_name, telegram_verified_at").eq("telegram_chat_id", chatId.toString()).single();
      if (!user) {
