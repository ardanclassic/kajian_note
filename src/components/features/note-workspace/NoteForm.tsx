...
interface NoteFormProps {
  note?: Note;
  initialValues?: Partial<CreateNoteFormData>; // For pre-populated data (e.g., from import)
  onSubmit: (data: CreateNoteFormData) => Promise<void>;
  onCancel?: () => void;
  isSubmitting?: boolean;
  mode?: "create" | "edit"; // For better clarity
  maxLength?: number; // Optional max character limit (default: 50K, Deep Note: 100K)
}
...
export function NoteForm({ note, initialValues, onSubmit, onCancel, isSubmitting = false, mode, maxLength = 50000 }: NoteFormProps) {
  ...
  const [content, setContent] = useState(initialValues?.content || note?.content || "");
  ...
  const {
    ...
    defaultValues: {
      title: initialValues?.title || note?.title || "",
      content: initialValues?.content || note?.content || "",
      isPublic: initialValues?.isPublic ?? note?.isPublic ?? false,
      tags: initialValues?.tags || note?.tags || [],
    },
  });
  ...
  useEffect(() => {
    ...
    if (content.length < 10) {
      ...
      setError("content", {
        type: "manual",
        message: `Konten maksimal ${maxLength.toLocaleString()} karakter`,
      });
    } else {
      clearErrors("content");
    }
  }, [content, setValue, setError, clearErrors, maxLength]);
